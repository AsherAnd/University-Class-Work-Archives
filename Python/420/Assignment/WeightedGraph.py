...
class WeightedGraph(object):
    ...
    def __init__(self, vertices, edges):
        ...
        for i in range(vertices):
            ...
            for j in range(i + 1, vertices):
                if edges[i][j] < 0:
                    print(
                        "Error: Weight of edges cannot be negative - negative weight was removed."
                    )
                    edges[i][j] = 0
                    edges[j][i] = 0
    ...
    def getEdge(self, sourceV, destV):
        ...
        if (
            sourceV >= 0
            and sourceV < self.totalV
            and destV >= 0
            and destV < self.totalV
        ):
    ...
    def isSubgraph(self, graph):
        ...
        """
        for row in range(self.totalVertices()):
            if not set(self.getEdges()[row]).issubset(set(graph.getEdges()[row])):
                return False
        return True
